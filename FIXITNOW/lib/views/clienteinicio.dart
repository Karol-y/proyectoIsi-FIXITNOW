      ...
      appBar: AppBar(
        leading: IconButton(
          ...
          onPressed: () {
            Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => const MenuScreenClient()),
            );
          },
        ),
        actions: [
  ...
  Future<List<Widget>> _getFilteredWorkers() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        workerTiles = workers.map((worker) {
          String imageUrl = worker['foto'] ?? '';
          return _buildWorkerTile(
            '${worker['nombres']} ${worker['apellidos']}',
            worker['tipSer'],
            imageUrl,
            worker['telefono'] ?? 'No disponible',
            worker['email'] ?? 'No disponible',
            worker['descripcion'] ?? 'Sin descripción',
          );
        }).toList();
      } else {
        throw Exception('Error al obtener los trabajadores: ${response.statusCode}');
